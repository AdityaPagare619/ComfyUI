...
@pytest.mark.parametrize("legitimate_path", [
    ...
    "data-2026-05-15.csv",
    "a" * 255,  # very long but legitimate path
])
def test_allows_legitimate_paths(legitimate_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            # Should return a valid path joined with input directory
...
def test_respects_annotation_overrides():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        set_input_directory(temp_dir)
        try:
            # [output] annotation should redirect to output dir
            result = get_annotated_filepath("test.png[output]")
            assert "[output]" not in result  # annotation stripped
